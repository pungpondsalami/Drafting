...
impl From<(f64, f64)> for Point {
    fn from(p: (f64, f64)) -> Self { Point { x: p.0, y: p.1 } }
}
...
pub enum RenderCommand {
    Line { x1: f64, y1: f64, x2: f64, y2: f64, is_selected: bool },
    Circle { cx: f64, cy: f64, radius: f64, is_selected: bool },
}
...
impl Camera {
    ...
    pub fn zoom_to_point(&mut self, world_x: f64, world_y: f64, screen_size: (f64, f64), factor: f64) {
...
impl Engine {
    ...
    pub fn move_selected(&mut self, dx: f64, dy: f64) {
        let selected_ids: Vec<u64> = self.history.iter()
            .filter(|e| e.is_selected)
            .map(|e| e.id)
            .collect();

        for el in &mut self.history {
            if !selected_ids.contains(&el.id) { continue; }
            match &mut el.data {
                ElementData::Line { start, end } => {
                    start.x += dx; start.y += dy;
                    end.x += dx; end.y += dy;
    ...
    pub fn move_and_snap_selected(&mut self, dx: f64, dy: f64, snap_enabled: bool) {
        ...
        let ref_point = self.history.iter()
    ...
    pub fn select_in_box(&mut self, x1: f64, y1: f64, x2: f64, y2: f64, crossing: bool, add_to_selection: bool) {
        ...
        for el in &mut self.history {
            if !hit_ids.contains(&el.id) { continue; }
            if crossing {
                el.is_selected = true;
            } else {
                let bb = el.bounding_box();
                if bb.min_x >= sel_box.min_x && bb.max_x <= sel_box.max_x
                && bb.min_y >= sel_box.min_y && bb.max_y <= sel_box.max_y {
                    el.is_selected = true;
                }
            }
        }
    ...
    pub fn get_array_preview_grid(
        ...
        let selected_elements: Vec<&DrawingElement> = self.history.iter()
            .filter(|e| e.is_selected)
            .collect();

        if selected_elements.is_empty() { return previews; }
                    ...
                    match &el.data {
                        ElementData::Line { start, end } => {
                            ...
                            previews.push(RenderCommand::Line {
                                x1: s_screen.0, y1: s_screen.1, x2: e_screen.0, y2: e_screen.1,
                                is_selected: true,
                            });
                        }
                        ElementData::Circle { center, radius } => {
                            let c = self.camera.world_to_screen(center.x + offset_x, center.y + offset_y);
                            // คูณ anim_scale เข้าไปเพื่อให้มันมี animation ขยายตัว
                            let r_screen = radius * self.camera.scale * self.array_settings.anim_scale;

                            previews.push(RenderCommand::Circle {
                                cx: c.0, cy: c.1, radius: r_screen,
                                is_selected: true,
                            });
                        }
                    }
    ...
    pub fn get_snapped_pos(&self, wx: f64, wy: f64, is_enabled: bool) -> (f64, f64) {
        ...
        let snap_coord = |coord: f64, major: f64, minor: f64| {
            ...
            if dist_to_major < (major * 0.3) { nearest_major }
            else { (coord / minor).round() * minor }
        };

        (snap_coord(wx, step_major, step_minor), snap_coord(wy, step_major, step_minor))
    }
    ...
    pub fn update_auto_pan(&mut self, x: f64, y: f64, width: f64, height: f64) {
        ...
        if x < margin { dx = 10.0; }
        else if x > width - margin { dx = -10.0; }
        if y < margin { dy = 10.0; }
        else if y > height - margin { dy = -10.0; }
    ...
    pub fn get_render_commands(&self, screen_w: f64, screen_h: f64) -> Vec<RenderCommand> {
        ...
        for el in visible_elements {
            ...
            if self.apply_anim.is_playing {
                if let Some(idx) = self.apply_anim.elements_to_reveal
                    .iter().position(|&id| id == el.id)
                {
                    let start_time = idx as f64 * self.apply_anim.stagger_delay;
                    let progress = ((self.apply_anim.timer - start_time) / 0.2)
                        .clamp(0.0, 1.0);

                    if progress <= 0.0 { continue; } // ยังไม่ถึงเวลา
