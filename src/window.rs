...
use crate::engine::{ToolMode as DrawingMode, DrawingElement, ElementData, Point, RenderCommand};
use dxf;
use std::io::Cursor;
...
mod imp {
    ...
    impl ObjectImpl for DraftingWindow {
        fn constructed(&self) {
            ...
            key_ctrl.connect_key_pressed(glib::clone!(#[weak] obj, #[upgrade_or] glib::Propagation::Proceed, move |_, key, _, state| {
                ...
                if key == gdk::Key::z || key == gdk::Key::Z {
                    if let Some(page) = obj.imp().tab_view.selected_page() {
                        let (w, h) = (
                            page.child().width() as f64,
                            page.child().height() as f64
                        );
                        let pages = obj.imp().pages.borrow();
                        if let Some(d_ref) = pages.get(&page) {
                            let mut d = d_ref.borrow_mut();
            ...
            self.btn_apply_array.connect_clicked(glib::clone!(#[weak] obj, move |_| {
                let imp = obj.imp();
            ...
            self.spin_rows.connect_value_changed(glib::clone!(#[weak] obj, move |spin| {
                if let Some(page) = obj.imp().tab_view.selected_page() {
                    let pages = obj.imp().pages.borrow();
                    if let Some(d_ref) = pages.get(&page) {
                        let mut d = d_ref.borrow_mut();
            ...
            self.spin_cols.connect_value_changed(glib::clone!(#[weak] obj, move |spin| {
                if let Some(page) = obj.imp().tab_view.selected_page() {
                    let pages = obj.imp().pages.borrow();
                    if let Some(d_ref) = pages.get(&page) {
                        let mut d = d_ref.borrow_mut();
            ...
            self.spin_spacing_x.connect_value_changed(glib::clone!(#[weak] obj, move |spin| {
                if let Some(page) = obj.imp().tab_view.selected_page() {
                    let pages = obj.imp().pages.borrow();
                    if let Some(d_ref) = pages.get(&page) {
                        let mut d = d_ref.borrow_mut();
            ...
            self.spin_spacing_y.connect_value_changed(glib::clone!(#[weak] obj, move |spin| {
                if let Some(page) = obj.imp().tab_view.selected_page() {
                    let pages = obj.imp().pages.borrow();
                    if let Some(d_ref) = pages.get(&page) {
                        let mut d = d_ref.borrow_mut();
            ...
            self.btn_snap.connect_toggled(glib::clone!(#[weak] obj, move |btn| {
            ...
            // ลบ Tab
            self.tab_view.connect_close_page(glib::clone!(#[weak] obj, #[upgrade_or] glib::Propagation::Proceed, move |_view, page| {
            ...
            // --- UI Actions ---
            self.btn_new.connect_clicked(glib::clone!(#[weak] obj, move |_| {
                obj.create_new_tab("Untitled");
            }));

            self.btn_line.connect_clicked(glib::clone!(#[weak] obj, move |_| {
                obj.imp().current_mode.set(DrawingMode::Line);
            }));

            self.btn_circle.connect_clicked(glib::clone!(#[weak] obj, move |_| {
                obj.imp().current_mode.set(DrawingMode::Circle);
            }));

            self.btn_undo.connect_clicked(glib::clone!(#[weak] obj, move |_| obj.undo_action()));
            self.btn_redo.connect_clicked(glib::clone!(#[weak] obj, move |_| obj.redo_action()));
            self.btn_save.connect_clicked(glib::clone!(#[weak] obj, move |_| obj.show_save_dialog()));

            self.btn_cancel_array.connect_clicked(glib::clone!(#[weak] obj, move |_| {
                let imp = obj.imp();
                if let Some(page) = imp.tab_view.selected_page() {
                    let pages = imp.pages.borrow();
                    if let Some(d_ref) = pages.get(&page) {
            ...
            self.btn_array.connect_clicked(glib::clone!(#[weak] obj, move |_| {
                let imp = obj.imp();
            ...
            self.cmd_entry.connect_activate(glib::clone!(#[weak] obj, move |entry| {
...
impl DraftingWindow {
    ...
    pub fn create_new_tab(&self, title: &str) {
        ...
        let canvas = gtk::DrawingArea::builder()
            .hexpand(true).vexpand(true).focusable(true).can_focus(true)
        ...
        imp.pages.borrow_mut().insert(page.clone(), RefCell::new(data));

        // --- 1. Zoom (Scroll) ---
        let scroll = gtk::EventControllerScroll::new(gtk::EventControllerScrollFlags::VERTICAL);
        scroll.connect_scroll(glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, #[upgrade_or] glib::Propagation::Stop, move |_, _, dy| {
            let pages = obj.imp().pages.borrow();
            if let Some(d_ref) = pages.get(&page) {
                let mut d = d_ref.borrow_mut();
                let (mx, my) = obj.imp().mouse_pos.get();
        ...
        canvas.add_tick_callback(glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, #[upgrade_or] glib::ControlFlow::Continue, move |area, clock| {
            let pages = obj.imp().pages.borrow();
            if let Some(d_ref) = pages.get(&page) {
                let mut d = d_ref.borrow_mut();
                ...
                if obj.imp().is_cancelling_array.get()
                    && d.array_settings.anim_scale < 0.01
                {
        ...
        motion.connect_motion(glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, #[weak] canvas, move |controller, x, y| {
            ...
            if let Some(d_ref) = pages.get(&page) {
                ...
                if obj.imp().current_mode.get() == DrawingMode::Move {
                    ...
                    let is_pressed = controller.current_event_state()
        ...
        click.connect_pressed(glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, #[weak] canvas, move |_, _, x, y| {
            canvas.grab_focus();
            let imp = obj.imp();
        ...
        click.connect_released(glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, move |gesture, _, x, y| {
            let is_shift = gesture.current_event_state()
        ...
        // --- 5. Draw Function ---
        canvas.set_draw_func(glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, move |_, cr, w, h| {
    ...
    fn draw_page(&self, page: &adw::TabPage, cr: &gtk::cairo::Context, w: f64, h: f64) {
        let imp = self.imp();
        let pages = imp.pages.borrow();
        let d_ref = match pages.get(page) { Some(r) => r, None => return };
        let d = d_ref.borrow();
        ...
        let draw_grid_line = |s: f64, alpha: f64, lw: f64| {
            ...
            while x_cursor <= w_max_x {
                ...
                cr.move_to(sx, 0.0); cr.line_to(sx, h);
                x_cursor += s;
            }
            ...
            while y_cursor <= w_max_y {
                ...
                cr.move_to(0.0, sy); cr.line_to(w, sy);
                y_cursor += s;
            }
            let _ = cr.stroke();
        };
        ...
        cr.set_source_rgb(1.0, 0.0, 0.0); cr.set_line_width(1.0);
        cr.move_to(cx, 0.0); cr.line_to(cx, h);
        cr.move_to(0.0, cy); cr.line_to(w, cy);
        let _ = cr.stroke();
        ...
        let padding = 10.0;
        let (box_w, box_h) = (text_w as f64 + padding * 2.0, text_h as f64 + padding * 2.0);

        cr.set_source_rgba(0.0, 0.0, 0.0, 0.7);
        cr.rectangle(10.0, 10.0, box_w, box_h);
        let _ = cr.fill();

        cr.set_source_rgb(1.0, 1.0, 0.0);
        cr.move_to(10.0 + padding, 10.0 + padding);
    ...
    pub fn show_save_dialog(&self) {
        if let Some(page) = self.imp().tab_view.selected_page() {
            ...
            dialog.save(None::<&gtk::Window>, gtk::gio::Cancellable::NONE, glib::clone!(#[weak(rename_to = obj)] self, #[weak] page, move |res| {
                if let Ok(file) = res {
                    let pages = obj.imp().pages.borrow();
                    if let Some(d_ref) = pages.get(&page) {
                        ...
                        let filename = file.basename()
                            .unwrap_or_else(|| std::ffi::OsString::from("drawing.dxf").into());
